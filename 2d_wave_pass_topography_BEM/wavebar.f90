...
!===============================================================================
      PROGRAM WATER_WAVE
      IMPLICIT NONE
	  INTEGER I,J,NT,NGA,NTIM,NFIELD,NPL,WTYP,NNODE,NELM,ICON,NITER,NWG
	  INTEGER,ALLOCATABLE::NELEM(:),ME(:),NS(:),BUNTYP(:),LN(:,:)
	  REAL DEP,D_OUT,GRAV,MU,WIDTH,THO,AMP,OMEGA,PSI,DELTTIME,SOR,WGX(10)
...
!---TOPOGRAGHY AND WAVE TYPE
	CALL WAVE_GEN(NPL,WTYP,NWG,WGX)
...
!---INPUT ALL KINDS OF PARAMETERS
	CALL INPUT(NPL,COOR,NFIELD,NNODE,NELM,NELEM,ME,NS,BUNTYP,DEP,D_OUT,NGA,GRAV,MU,WIDTH,THO,&
...
!	   CALL SOLVE_SOR(NPL,PHI,PPHI,KER1,KER2,BUNTYP,NNODE,SOR,NELEM)
...
!	  CALL SOLVE_SOR(NPL,PHIT,PPHIT,KER1,KER2,BUNTYP,NNODE,SOR,NELEM)
...
!---READ TIME,TIME STEP,GRAV ACC.,FREQUENCY,AMPLITUDE
		...
		DEP=COOR(NPL,2)
		D_OUT=COOR(NPL-1,2)-COOR(NPL-2,2)
...
END DO
WRITE(7,"(11(E15.8,1X))") TIME,WGY(1:NWG)*100.0
...
!**** CALCULATE THE JACOBIAN
      DO J=1,NELM
      LENG(J)=((NODE(LN(J,1),1)-NODE(LN(J,2),1))**2+&
			  &(NODE(LN(J,1),2)-NODE(LN(J,2),2))**2)**0.5
		DO L=1,2
      ...
      DO I = 1,NNODE
       ...
       END DO
		...
		ELSE !---NON SINGULER TERM
         G(K)=0.0
         DO M=1,NGA
            H(K)=H(K)+(-1)/((XFUNC(M)-NODE(I,1))**2+(YFUNC(M)-NODE(I,2))**2)*&
					&((XFUNC(M)-NODE(I,1))*NORM(J,1)+(YFUNC(M)-NODE(I,2))*NORM(J,2))&
					&*JCB(J)*SH(K,M)*WT(M)
            G(K)=G(K)+LOG(1./((XFUNC(M)-NODE(I,1))**2+(YFUNC(M)-NODE(I,2))**2)**0.5)*&
					&JCB(J)*SH(K,M)*WT(M)
         END DO
      END IF
...
!*************GSI SOR INTERATIVE
      ...
      IF(PMAX .EQ. 0) THEN
        IF (EMAX .LE. 0.0000001) EXIT
      ELSE
        ETOL=EMAX/PMAX
        IF (ETOL .LE. 0.0001) EXIT
      END IF
      END DO
  100 WRITE(*,*) 'L',L
...
!********************************************************************
	...
	REAL DPDS(NNODE),PHIT(NNODE),DPDT(NNODE)
...
!**********************************************************************
      ...
      REAL NODE(NNODE,2),PHI(NNODE),PPHI(NNODE),PHIT(NNODE),PPHIT(NNODE)
	  REAL DP(NNODE,2),DPDS(NNODE),DPDT(NNODE)
...
!----CREATE DOMAIN POINT
	...
	DO I=2,NS(1)-1
	  ...
	  HB=NODE(IL,2)+(NODE(I,1)-NODE(IL,1))/(NODE(IR,1)-NODE(IL,1))*(NODE(IR,2)-NODE(IL,2))+0.01 ! keep it a little far away from the boundary
         ...
         H(K)=0.0
          DO M=1,NGA
            ...
            H(K)=H(K)+(((YFUNC(M)-DNODE(I,2))**2-(XFUNC(M)-DNODE(I,1))**2)*NORM(J,1)-&
					&2.0*(YFUNC(M)-DNODE(I,2))*(XFUNC(M)-DNODE(I,1))*NORM(J,2))/&
         ...
         H(K)=0.0
          DO M=1,NGA
            ...
            H(K)=H(K)+(((XFUNC(M)-DNODE(I,1))**2-(YFUNC(M)-DNODE(I,2))**2)*NORM(J,2)-&
					&2.0*(YFUNC(M)-DNODE(I,2))*(XFUNC(M)-DNODE(I,1))*NORM(J,1))/&
					&((XFUNC(M)-DNODE(I,1))**2+(YFUNC(M)-DNODE(I,2))**2)**2*TEMP
       ...
       DO J=1,NELM
		...
		DO K=1,2
         ...
         H(K)=0.0
          DO M=1,NGA
			TEMP=JCB(J)*SH(K,M)*WT(M)
            H(K)=H(K)+(-1)/((XFUNC(M)-DNODE(I,1))**2+(YFUNC(M)-DNODE(I,2))**2)*&
...
!********************************************************************
	...
	K=1.0
	DO I=1,100
	F0=K*TANH(K*D)-(2*PI/T)**2/GRAV
	F1=TANH(K*D)+K-K*(TANH(K*D)**2)
	K2=K-(F0)/(F1)
		IF((K2-K)/K<=0.000001) THEN
		EXIT
		END IF
	K=K2
	END DO
